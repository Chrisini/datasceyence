...
class DecentBlock_Early_Shuffle_MLP(nn.Module):
    ...
    def __init__(self, out_features=128):
        ...
        self.mlp_head = nn.Sequential(
            nn.Linear(encoder_out, encoder_out),
            nn.ReLU(inplace=True),
            nn.Linear(encoder_out, self.out_features)
        )
    ...
    def forward(self, x):
        x = self.encoder(x)
        x = self.mlp_head(x)
        block_output = nn.functional.normalize(x, dim=1)
        return block_output
    
class DecentBlock_Early_Shuffle_116OutChannels(nn.Module):
    # =============================================================================
    # use this
    # encoder backbone (shufflenet_v2_x1_0) + projection head (MLP)
    ...
    def __init__(self, ckpt_early_blocks_path, ckpt_early_blocks, out_channels=5):
        super(DecentBlock_Shuffle_EarlyBlock, self).__init__()
        ...
        # needs to be in nn.Seq for some reason ...
        # frozen
        self.decent_block_116 = nn.Sequential(*(list(decent_block.children())[:-4])).to(device)
        self.decent_block_116.requires_grad_(False) # todo check whether this works

        # trainable
        self.decent_block_reduction = nn.Sequential(
                                              nn.Conv2d(116, self.out_channels, (1, 1), stride=(1, 1), padding=(0, 0), dilation=(1, 1)),
...
class DecentBlock_Early_Shuffle_Linear(nn.Module):
    # =============================================================================
    ...
    def __init__(self, out_classes=10):
        ...
        # encoder without fully connected classification head (linear layer)
        self.encoder = shufflenet
        self.encoder.fc = nn.Linear(encoder_out, self.out_classes)
