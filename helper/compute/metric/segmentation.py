...
class SegmentationMetrics(object):
    ...
    def _calculate_multi_metrics(self, gt, pred, class_num):
        ...
        jaccard = (matrix[0] + self.eps) / (matrix[0] + matrix[1] + matrix[2] + self.eps)  

        if self.average:
            dice = np.average(dice)
            precision = np.average(precision)
            recall = np.average(recall)
            jaccard = np.average(jaccard)

        return pixel_acc, dice, precision, recall, jaccard
    ...
    def __call__(self, y_true, y_pred):
        ...
        pixel_acc, dice, precision, recall, jaccard = self._calculate_multi_metrics(gt_onehot, activated_pred, class_num)
        ...
        return item # pixel_acc, dice, precision, recall
